...
window.addEventListener("DOMContentLoaded", () => {
  ...
  getResource("http://localhost:3333/menu").then(data => {
    data.forEach(({
      ...
      new MenuCard(img, altimg, title, descr, price, ".menu .container").render(); // ".menu .container" прописываем родителя куда добавляем
    });
  ...
  function bindPostData(form) {
    ...
    form.addEventListener("submit", e => {
      ...
      postData("http://localhost:3333/requests", json).then(data => {
  ...
  slider.style.position = "relative"; // add Dots for Slider
